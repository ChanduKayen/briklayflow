-- ============================================================
-- Add sub_category, dimension, variant, grade to trgm_match_sku return.
--
-- Enhancement 1 (spec auto-fill) needs these fields individually so the
-- frontend can build a "12mm · Fe500D" spec string without a second lookup.
-- These columns are already fetched inside filtered_skus — this migration just
-- exposes them in the RETURNS TABLE.
--
-- No scoring logic changes — this is a pure return-signature extension.
-- ============================================================
  ...
  scored AS (
    SELECT
      ...
      greatest(
        ...
        -- Signal C: reverse word_similarity
        word_similarity(s.item_name, (SELECT q FROM norm)),
        -- Signal D: per-alias MAX — "safeda" vs alias["SAFEDA"] → 1.0
        COALESCE(
          (SELECT MAX(GREATEST(
