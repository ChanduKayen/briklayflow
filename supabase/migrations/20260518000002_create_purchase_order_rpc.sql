-- PR-101 B: wrap purchase_orders + po_line_items into one atomic transaction
CREATE OR REPLACE FUNCTION public.create_purchase_order(
  p_po_data    jsonb,   -- all purchase_order fields (org_id, po_id, project_id, ...)
  p_line_items jsonb    -- [{line_number, category_id, item_name, specification, unit,
                        --   quantity_ordered, unit_rate, basic_amount, discount_percent,
                        --   discount_amount, gst_rate, cgst, sgst, igst, total_amount}]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  RETURN jsonb_build_object('success', true, 'po_id', p_po_data->>'po_id');
