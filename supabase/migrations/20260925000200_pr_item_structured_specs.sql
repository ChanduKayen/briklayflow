-- ===========================================================================
-- Standardize purchase-request items: carry the STRUCTURED specs — size (width ×
-- height in mm), specification, brand — as their own fields, not folded into note.
-- purchase_request_items already has width_mm / height_mm / spec / brand columns
-- (the review card reads them); staging simply wasn't writing them. This re-emits
-- stage_purchase_request so p_items may include width_mm, height_mm, spec, brand,
-- and they land on the row. Older callers that omit them are unaffected (NULL).
-- ===========================================================================
...
CREATE OR REPLACE FUNCTION public.stage_purchase_request(
  ...
  p_title         text,
  p_items         jsonb,          -- [{ item_name, quantity, unit, width_mm, height_mm, spec, brand, note }]
...
BEGIN
  ...
  LOOP
    INSERT INTO public.purchase_request_items
      (purchase_request_id, org_id, item_index, item_name, quantity, unit, width_mm, height_mm, spec, brand, note)
    VALUES
      (v_id, p_org_id, v_idx,
       v_item->>'item_name',
