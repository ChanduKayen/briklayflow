-- ===========================================================================
-- Day Book "split into multiple transactions" — per-row payee / category / remarks.
--
-- insert_split_transactions shared payee/category/remarks across every split (p_base).
-- Generalize it so each split may carry its OWN stakeholder_id, category and remarks,
-- falling back to p_base when a split omits them. Backward compatible: the existing
-- split-among-sites caller (which only varies project + amount) still works unchanged —
-- its splits inherit the base payee/category/remarks via COALESCE.
-- ===========================================================================
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.transactions (
      ...
      v_txn_id,
      -- per-row payee overrides the shared one; '' / null → general expense (no party)
      ...
      COALESCE(v_split->>'category', p_base->>'category'),
      COALESCE(v_split->>'remarks',  p_base->>'remarks'),
      p_base->>'bill_doc_url',
