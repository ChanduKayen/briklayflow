...
- NEW_INTENT: a NEW actionable task, EVEN IF something is pending (never scold).
    TRANSACTION (a NARROW exception) = a payment/expense, including a material ALREADY bought with an amount. Recognized by an amount together with a party or a payment verb. Past-tense purchase verbs mean ALREADY bought -> TRANSACTION: Telugu konnam/konnaru, Hindi liya/khareeda, English bought/paid ("ramu 5000 cash", "paid suresh 15000", "cement 50 bags konnam 7000", "mistri ko 8000 diya").
      A VENDOR BILL OR INVOICE also belongs to TRANSACTION — a financial document a vendor issued (a bill / invoice / tax invoice showing a vendor and an amount owed), EVEN IF it has not been paid and the message names no payment. This agent owns the whole money document; it decides for itself whether it is only a bill to record or a bill that was also paid, so route it here rather than treating an unpaid invoice as site talk.
    PROCUREMENT (a NARROW exception) = a message whose PURPOSE is to OBTAIN materials not yet purchased. Recognized by ANY of:
      (a) a buy / order / arrange / need instruction with a material — "order 100 bags cement", "need 20 ton sand", "arrange tmt steel", "need iron for shyam site", "cement kavali";
      (b) a MATERIALS LIST / INDENT — a material with a quantity, or two-or-more materials listed to procure — EVEN WITH NO buy word. A site sends its purchase list as a bare indent: "16mm steel 10, 12mm 15, 8mm 60, 20kg binding wire", "cement 50 bags, sand 2 units, tiles 40 boxes", a list of switch boxes / pipes / fittings. A bare list of materials to get IS an order;
      (c) a stated NEED or REQUIREMENT of a material, even a single one — "need cement", "iron kavali", "5 window frames required".
      A list of materials, or just the site's materials, or a need for them, is ALWAYS procurement — whether or not a buy word appears.
    ...
    SITEOPS (the DEFAULT / residual) = anything REPORTING site state that is NOT a payment, NOT an order and NOT a question: work progress or completion, problems / blockers, material or labour situations, attendance, site to-dos — in any form, however terse or context-poor, in any language including native script. Do NOT rely on resemblance to examples; SITEOPS is whatever remains after the narrow exceptions are excluded. A single message mixing progress AND a problem is STILL SITEOPS.
      THE ONE BOUNDARY THAT NEEDS CARE — a material can appear TWO ways:
        • TO OBTAIN (PROCUREMENT): the message exists to GET the materials — a request, a need, or a BARE INDENT LIST. "order cement", "need 100 bags", "arrange sand", or a plain list "16mm 10, 12mm 15, 8mm 60, binding wire 20kg". A pure list of materials / quantities, with no problem or work-state words, is ALWAYS procurement — the absence of a buy verb does NOT make it site talk.
        • AS A SITE STATE (SITEOPS): the material is part of a PROBLEM, a BLOCKAGE, a DELIVERY/RECEIPT, or a WORK/PROGRESS description — "cement short" / "no sand left" / "steel didn't come", "slab work stuck, cement not enough", "received 50 bags cement at site", "3rd floor wiring pending, conduit not laid". Here the material DESCRIBES a situation; it is not an order.
      TEST: does the message LIST or ASK FOR materials to procure (→ PROCUREMENT), or does it DESCRIBE a work state, a problem, a blockage or a delivery that mentions a material (→ SITEOPS)? Materials + a related work / blockage / description = SITEOPS; a bare list or need of materials = PROCUREMENT.
