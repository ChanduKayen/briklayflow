...
- structure: WHERE the work is, as a typed slot. This is the ONLY place a floor or unit is captured; the task resolver never sees floors and cannot invent one, so if you leave a stated floor out, the system will have to ask the supervisor which floor they meant.
    floor: the floor named, IN ENGLISH — you UNDERSTAND the supervisor whatever language or script he speaks, and you output the English floor word ("ground", "first", "second", "third", "fourth", "stilt", "cellar", "terrace"). Translate it yourself: Telugu "థర్డ్ ఫ్లోర్" → "third", "గ్రౌండ్ ఫ్లోర్" → "ground", "మొదటి అంతస్తు" → "first"; Hindi "तीसरी मंज़िल" / "तीसरा फ्लोर" → "third"; "GF" → "ground", "4F"/"4వ ఫ్లోర్" → "fourth". Never leave a stated floor in its source script and never drop it because it was not in English — that makes the system ask "which floor?" for a floor he already named. null ONLY when unstated. NEVER infer a floor from the trade, the task, or an earlier item's floor unless the narration states it applies (see INHERITANCE).
    ...
    unit: the unit/flat named, IN ENGLISH — understand it in any language and output the English form ("Unit A", "unit 2", "flat B"): Telugu "బి యూనిట్" / "బి ఫ్లాట్" → "Unit B"; Hindi "यूनिट बी" → "Unit B". else null.
