...
/** The whole-name scorer (mirror of scoreName's people rules) but BOUNDARY-AWARE: containment counts only
 *  across WHOLE WORDS, so "siva" inside "sivaramu" no longer scores 0.9. The fuzzy tail (first-word, 1–2
 *  edits, relative distance) is kept, so a romanised near-name still whispers (laxmi→Lakshmi ≈ 0.57). */
...
export function scorePayeeRich(q: string, cand: { name: string; type?: string | null; category?: string | null; aliases?: string[] | null }): number {
  ...
  // Floor with a BOUNDARY-AWARE whole-name check, not scoreName's raw substring — the latter scored
  // "Siva" a 0.9 match for "Darlanka Sivaramu" (siva sits inside sivaramu). The token bag carries the rest.
  let score = Math.max(nameTokenScore(qName, payeeTokens(cl)), wholeNameFloor(query, cl))
  if (qRoles.length) {
