...
export async function extractProcurementFromImage(
  ...
  const prompt =
    ...
    `LANGUAGE — the list may be written in ANY language or script (Telugu, Hindi, Tamil, Kannada, Marathi, Bengali, Urdu, Odia, a mix, or Roman-script phonetic), and often in a site worker's COLLOQUIAL / SLANG trade words, not the textbook term. Always OUTPUT item_name, unit, note and title in ENGLISH — the plain word a builder/supplier writes on an order:\n` +
    `  · Translate the material to its standard English term, whatever the language or spelling: సిమెంట్ / सीमेंट / சிமெண்ட் → "cement"; ఇసుక / रेत / ரெத் → "sand"; కంకర / जेली / "jelly" → "aggregate"; కడ్డీలు / సळ్ళు / सरिया / "gaadi" → "steel bars"; ఇటుకలు / ईंट → "bricks"; టైల్స్ → "tiles"; పైపులు / "pype" → "pipes"; రంగు / पेंట → "paint"; సెంట్రింగ్ → "centering / shuttering"; "kirchi / kankara" → "aggregate".\n` +
    `  · Map colloquial / slang / phonetic spellings to the real material by MEANING, not letters — a mason's spoken word, a shortened form, or a regional name for a fitting all resolve to the standard item. Units too: సంచులు / बोरी / "borey" → "bags"; టన్ను → "ton"; బ్రాస్ → "brass"; క్యూబిక్ ఫీట్ → "cft"; నంబర్లు / పీసులు / "peice" → "nos".\n` +
    `  · Keep a BRAND or proper noun transliterated in Roman letters (Ultratech, Ramco, Finolex, Asian Paints, a shop or person's name) — never translate a brand into a generic word.\n` +
    `  · NEVER invent. If a word is not clearly a known material, transliterate it FAITHFULLY to Roman letters rather than guessing a different item; do not add a spec, grade or quantity the sheet does not show.\n` +
