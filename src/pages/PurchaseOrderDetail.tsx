...
export default function PurchaseOrderDetail({ session }: { session: Session }) {
  ...
  // Attach a bill: ALWAYS open the popup — the vendor's already-uploaded bills to link (no duplicate), plus an
  // "Upload a new bill" that saves it to the Bills module and attaches it. A PO never uploads a bill of its own.
  const openPoBillPicker = () => setBillChoiceOpen(true);
  // Keep the Bills popup OPEN — the new bill lands in its grid (auto-selected) once it's saved to the module.
  const uploadNewBill = () => { poBillInputRef.current?.click(); };
  // Link an existing bill (no close — the popup shows its own success beat, then closes). Throws on error.
  const linkBillById = async (billId: string): Promise<void> => {
    if (!poId) return;
    try {
      await linkExistingBillToPO(billId, poId, po?.project_id ?? null);
      refetchPoBills();
  ...
  return (
    <div className={`podx${isMobile ? ' m' : ''}`} onClick={() => menuOpen && setMenuOpen(false)}>
      ...
      {/* Attach a bill: link one of the vendor's already-uploaded bills, or upload a new one (→ Bills module). */}
      {billChoiceOpen && (
        <PoAttachBillPopup
