...
const STATUS_LABEL: Record<string, string> = {
  ORDERED: 'Ordered', BILLED: 'Billed', PARTIAL: 'Partial', PAID: 'Paid', CANCELLED: 'Cancelled',
}

const ALL_STATUSES = ['ORDERED', 'BILLED', 'PARTIAL', 'PAID', 'CANCELLED']

type DatePreset = 'all' | 'today' | 'week' | 'month' | 'quarter'
const DATE_LABELS: Record<DatePreset, string> = { all: 'All Time', today: 'Today', week: 'Last 7 Days', month: 'Last Month', quarter: 'Last Quarter' }
...
function getItemsPreview(po: any): string {
  if (po.po_line_items?.length > 0) {
    return po.po_line_items.slice(0, 3).map((li: any) => li.item_name).join(', ') + (po.po_line_items.length > 3 ? ` +${po.po_line_items.length - 3} more` : '')
  }
  return ''
}
...
function dateRange(preset: DatePreset): { from: Date | null; to: Date | null } {
  ...
  if (preset === 'week') { const s = new Date(); s.setDate(s.getDate() - 7); return { from: s, to: new Date() } }
  if (preset === 'month') { const s = new Date(); s.setMonth(s.getMonth() - 1); return { from: s, to: new Date() } }
  if (preset === 'quarter') { const s = new Date(); s.setMonth(s.getMonth() - 3); return { from: s, to: new Date() } }
  return { from: null, to: null }
}

function FilterChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`bk-chip inline-flex items-center gap-1 px-3 h-7 rounded-full text-[12px] font-medium transition-colors whitespace-nowrap ${active ? 'bg-primary/[0.08] text-primary' : 'text-on-surface-variant/60 hover:bg-black/[0.04]'}`}>
      {label}
      <span className="material-symbols-outlined text-[13px]">{active ? 'expand_less' : 'expand_more'}</span>
    </button>
  )
}

function Dropdown({ anchorRef, open, onClose, children }: { anchorRef: React.RefObject<HTMLElement>; open: boolean; onClose: () => void; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    function handler(e: MouseEvent) { if (!ref.current?.contains(e.target as Node) && !anchorRef.current?.contains(e.target as Node)) onClose() }
  ...
  return createPortal(
    <div ref={ref} className="popover-animate bg-white rounded-xl shadow-elevation-8 border border-outline-variant/20 py-1 min-w-[180px]" style={{ position: 'fixed', top: pos.top, left: pos.left, zIndex: 9999 }}>
      {children}
    </div>, document.body
  )
}

function VendorBillCell({ po, canManage, currentUserName, onUpdate, showSnackbar }: { po: any; canManage: boolean; currentUserName: string; onUpdate: (id: string, u: any) => void; showSnackbar: (m: string, o?: any) => void }) {
  ...
  const handleSave = async () => {
    ...
    if (!error) {
      setEditing(false)
      onUpdate(po.po_id, { vendor_bill_amount: parsed, vendor_bill_number: billNo.trim() || null, bill_recorded_at: new Date().toISOString() })
    ...
    } else { showSnackbar(error.message || 'Failed to save bill', { type: 'error' }) }
  }

  if (!canManage) return <td className="px-4 py-4">{hasBill ? <p className="text-[13px] font-semibold font-mono text-green-700 tabular-nums">₹{Number(po.vendor_bill_amount).toLocaleString('en-IN')}</p> : <span className="text-on-surface-variant/20 text-[12px]">—</span>}</td>
    ...
    <td className="px-3 py-2" onClick={e => e.stopPropagation()}>
      <div className="rounded-xl p-3 bg-white border-2 border-amber-300 shadow-[0_4px_20px_rgba(245,158,11,0.12)] relative z-10 w-[180px]">
        ...
        <div className="relative mb-2">
          ...
          <input type="number" placeholder="0" value={amount} onChange={e => setAmount(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') { setEditing(false); setAmount('') } }} className="w-full h-9 text-[17px] font-bold pl-7 pr-2 border border-amber-200 rounded-lg bg-amber-50/50 focus:outline-none focus:border-amber-400 focus:bg-white" autoFocus />
        </div>
        <input type="text" placeholder="Invoice no. (optional)" value={billNo} onChange={e => setBillNo(e.target.value)} className="w-full h-7 px-2.5 mb-2 text-[11px] border border-outline-variant/20 rounded-lg focus:outline-none focus:border-amber-300 bg-white" />
        <div className="flex gap-1.5">
          <button onClick={handleSave} disabled={!amount || parseFloat(amount) <= 0 || saving} className={`flex-1 h-7 rounded-lg text-[12px] font-semibold transition-colors ${amount && parseFloat(amount) > 0 ? 'bg-amber-500 text-white hover:bg-amber-600' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}>{saving ? '…' : 'Save'}</button>
          <button onClick={() => { setEditing(false); setAmount('') }} className="w-7 h-7 rounded-lg border border-outline-variant/15 text-on-surface-variant/40 hover:bg-surface-container flex items-center justify-center"><span className="material-symbols-outlined text-[13px]">close</span></button>
        </div>
      </div>
    </td>
  )
  ...
  if (!hasBill) return (
    <td className="px-4 py-4" onClick={e => e.stopPropagation()}>
      <button onClick={() => { setAmount(''); setBillNo(po.vendor_bill_number || po.vendor_bill_no || ''); setEditing(true) }} className="flex items-center gap-1 text-[12px] font-medium text-amber-500 hover:text-amber-700 transition-colors">
        <span className="material-symbols-outlined text-[14px]">add</span>Add bill
      </button>
    </td>
  )
  ...
  return (
    <td className="px-4 py-4" onClick={e => e.stopPropagation()}>
      <button onClick={() => { setAmount(String(po.vendor_bill_amount)); setBillNo(po.vendor_bill_number || po.vendor_bill_no || ''); setEditing(true) }} className="text-left group">
        <p className="text-[13px] font-semibold font-mono text-green-700 tabular-nums group-hover:underline">₹{Number(po.vendor_bill_amount).toLocaleString('en-IN')}</p>
        <p className="text-[11px] text-on-surface-variant/40 mt-0.5">{[po.vendor_bill_number || po.vendor_bill_no, po.bill_recorded_at && fmtShortDate(po.bill_recorded_at)].filter(Boolean).join(' · ') || 'Recorded'}</p>
      </button>
    </td>
  )
}

function BillUploadCell({ po, canManage, onUpdate, showSnackbar }: { po: any; canManage: boolean; onUpdate: (id: string, u: any) => void; showSnackbar: (m: string, o?: any) => void }) {
  ...
  return (
    <td className="px-4 py-4" onClick={e => e.stopPropagation()}>
      <label className={`flex items-center gap-1 text-[12px] font-medium transition-colors ${uploading ? 'text-on-surface-variant/30 cursor-wait' : 'text-on-surface-variant/35 hover:text-primary cursor-pointer'}`}>
        ...
        <input type="file" accept=".pdf,.jpg,.jpeg,.png" className="hidden" disabled={uploading} onChange={async (e) => {
          ...
          if (!upErr) {
            ...
            onUpdate(po.po_id, { vendor_bill_url: url }); showSnackbar('Bill document uploaded!')
          } else { showSnackbar(upErr.message || 'Upload failed', { type: 'error' }) }
...
function SiteReceiptCell({ po, canManage, currentUserName, currentUserId, onUpdate, showSnackbar }: { po: any; canManage: boolean; currentUserName: string; currentUserId: string; onUpdate: (id: string, u: any) => void; showSnackbar: (m: string, o?: any) => void }) {
  ...
  if (isReceived) return (
    <td className="px-4 py-4">
      <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-purple-400 shrink-0" /><p className="text-[12px] font-medium text-purple-700">At Site</p></div>
      <p className="text-[11px] text-on-surface-variant/40 mt-0.5 pl-3">{[po.received_by_name?.split(' ')[0], po.received_at_site && fmtShortDate(po.received_at_site)].filter(Boolean).join(' · ')}</p>
    </td>
  )

  if (!canManage || !hasBill) return <td className="px-4 py-4"><span className="text-on-surface-variant/20 text-[12px]">—</span></td>
      ...
      <div className="rounded-xl p-3 bg-white border-2 border-purple-200 shadow-lg relative z-10 w-[160px]">
        ...
        <div className="flex gap-1.5">
          <button onClick={async () => {
            ...
            const { error } = await supabase.from('purchase_orders').update({ received_at_site: now, received_by_name: currentUserName, received_by_user_id: currentUserId, status: 'BILLED' }).eq('po_id', po.po_id)
            setSaving(false); setConfirming(false)
            if (!error) { onUpdate(po.po_id, { received_at_site: now, received_by_name: currentUserName, status: 'BILLED' }); showSnackbar('Receipt confirmed — status updated to Billed') }
            else { showSnackbar(error.message || 'Failed to confirm receipt', { type: 'error' }) }
          }} disabled={saving} className="flex-1 py-1.5 rounded-lg bg-purple-600 text-white text-[11px] font-semibold hover:bg-purple-700 disabled:opacity-50">{saving ? '…' : 'Confirm'}</button>
          <button onClick={() => setConfirming(false)} className="px-2 py-1.5 rounded-lg border border-outline-variant/15 text-on-surface-variant/40 hover:bg-surface-container"><span className="material-symbols-outlined text-[12px]">close</span></button>
        </div>
      </div>
    </td>
  )
  ...
  return (
    <td className="px-4 py-4" onClick={e => e.stopPropagation()}>
      <button onClick={() => setConfirming(true)} className="flex items-center gap-1 text-[12px] font-medium text-purple-600 hover:text-purple-800 border border-purple-200 rounded-lg px-2.5 py-1 hover:bg-purple-50 transition-colors">
        <span className="material-symbols-outlined text-[13px]">done_all</span>Mark received
      </button>
    </td>
  )
}

export default function ProjectPurchaseOrders({ session }: { session: Session }) {
  ...
  const [openChip, setOpenChip] = useState<'date' | 'status' | 'vendor' | null>(null)
  ...
  const handlePOUpdate = (poId: string, updates: any) => {
    ...
    qc.setQueryData(['project_pos_enhanced', projectId], (old: any[] | undefined) => old?.map(p => p.po_id === poId ? { ...p, ...updates } : p))
  }
  ...
  const { data: rawPos = [], isLoading, isError } = useQuery({
    queryKey: ['project_pos_enhanced', projectId],
    queryFn: async () => {
  ...
  const pos = (rawPos as any[]).map(p => poOverrides[p.po_id] ? { ...p, ...poOverrides[p.po_id] } : p)
  const vendors = [...new Set(pos.map((p: any) => p.stakeholders?.name).filter(Boolean))].sort() as string[]

  const { from: dateFrom, to: dateTo } = dateRange(datePreset)

  const filtered = pos.filter((po: any) => {
    if (dateFrom && dateTo) { const d = new Date(po.date_issued || po.created_at); if (d < dateFrom || d > dateTo) return false }
    ...
    if (searchTerm) { const q = searchTerm.toLowerCase(); if (!(po.po_id?.toLowerCase().includes(q) || po.stakeholders?.name?.toLowerCase().includes(q) || po.ordered_by?.toLowerCase().includes(q))) return false }
    return true
  })

  const totalCount      = pos.length
  const pendingDelivery = pos.filter((p: any) => p.status === 'ORDERED').length
  const pendingBill     = pos.filter((p: any) => p.status === 'ORDERED' && !Number(p.vendor_bill_amount)).length
  const hasFilters = datePreset !== 'all' || filterStatus.length > 0 || filterVendor.length > 0 || !!searchTerm

  function clearFilters() { setDatePreset('all'); setFilterStatus([]); setFilterVendor([]); setSearchTerm(''); setSearchOpen(false) }
  function toggleChip(name: typeof openChip) { setOpenChip(c => c === name ? null : name) }

  function MultiCheckList({ options, selected, onChange }: { options: string[]; selected: string[]; onChange: (v: string[]) => void }) {
    return (
      <div className="py-1 max-h-64 overflow-y-auto">
        <button onClick={() => onChange([])} className="w-full text-left px-4 py-2 text-[12px] text-on-surface-variant/60 hover:bg-surface-container-low">All</button>
        {options.map(o => (
          <label key={o} className="flex items-center gap-2.5 px-4 py-2 text-[13px] hover:bg-surface-container-low cursor-pointer">
            <input type="checkbox" checked={selected.includes(o)} onChange={() => { if (selected.includes(o)) onChange(selected.filter(s => s !== o)); else onChange([...selected, o]) }} className="accent-primary w-3.5 h-3.5" />
            <span>{o}</span>
          </label>
        ))}
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[12px] text-on-surface font-semibold">Purchase Orders</span>
            </div>
            ...
            <p className="text-[12px] text-on-surface-variant/45 mt-1 leading-relaxed">
              {totalCount} orders
              {pendingDelivery > 0 && <> · <span className="text-amber-600">{pendingDelivery} pending delivery</span></>}
              {pendingBill > 0 && <> · <span className="text-orange-500">{pendingBill} need bill</span></>}
            </p>
          </div>
          {canManage && (
        ...
        <div className="flex items-center gap-0.5 overflow-x-auto no-scrollbar flex-nowrap md:flex-wrap border-b border-outline-variant/[0.08] pb-3 mb-4">
          <div ref={dateRef} className="relative">
            ...
            <Dropdown anchorRef={dateRef as React.RefObject<HTMLElement>} open={openChip === 'date'} onClose={() => setOpenChip(null)}>
              {(['all', 'today', 'week', 'month', 'quarter'] as DatePreset[]).map(p => (
                <button key={p} onClick={() => { setDatePreset(p); setOpenChip(null) }} className={`w-full text-left px-4 py-2 text-[13px] hover:bg-surface-container-low ${datePreset === p ? 'font-semibold text-primary' : 'text-on-surface'}`}>{DATE_LABELS[p]}</button>
              ))}
            </Dropdown>
          </div>
          ...
          {searchOpen ? (
            <div className="relative flex items-center">
              ...
              <input autoFocus className="pl-8 pr-7 h-7 rounded-full text-[12px] border border-outline-variant/25 bg-white outline-none focus:border-primary/40 w-36 transition-all" placeholder="Search…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              {searchTerm && <button onClick={() => setSearchTerm('')} className="absolute right-2 text-on-surface-variant/30 hover:text-on-surface"><span className="material-symbols-outlined text-[12px]">close</span></button>}
            </div>
          ) : (
        ...
        {!isLoading && isError && (
          <div className="text-center py-20">
            <span className="material-symbols-outlined text-[44px] block mb-3 text-on-surface-variant/15">error_outline</span>
            <p className="text-[14px] font-medium text-on-surface/40">Failed to load purchase orders</p>
            <p className="text-[12px] mt-1 text-on-surface-variant/30">There was a problem fetching data</p>
          </div>
        )}
        ...
        {!isLoading && !isError && filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-[14px] font-medium text-on-surface/40">{hasFilters ? 'No orders match your filters' : 'No purchase orders yet'}</p>
            <p className="text-[12px] mt-1 text-on-surface-variant/30">{hasFilters ? 'Try adjusting or clearing the filters' : 'Purchase orders will appear here once created'}</p>
            {hasFilters && <button onClick={clearFilters} className="mt-4 bk-btn text-[13px]">Clear filters</button>}
            {canManage && !hasFilters && <button onClick={() => navigate('/purchase-orders/new', { state: { projectId } })} className="mt-4 bk-btn text-[13px]">Create your first PO</button>}
          </div>
        )}
                ...
                {filtered.map((po: any, idx: number) => (
                  <tr key={po.po_id} className={`group border-b border-outline-variant/[0.05] last:border-0 wo-row-animate hover:bg-surface-container-low/25 transition-colors ${STATUS_BORDER[po.status] ?? 'border-l-[3px] border-l-transparent'}`} style={{ animationDelay: `${Math.min(idx, 20) * 15}ms` }}>
                    ...
                    <td className="px-4 py-4">
                      ...
                      {po.stakeholders?.category && <p className="text-[11px] text-on-surface-variant/40 mt-0.5">{po.stakeholders.category}</p>}
                      {getItemsPreview(po) && <p className="text-[11px] text-on-surface-variant/25 mt-0.5 truncate max-w-[200px]">{getItemsPreview(po)}</p>}
                    </td>
                    <td className="px-4 py-4 w-[100px]">
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-md ${STATUS_CHIP[po.status] ?? 'bg-surface-container text-on-surface-variant'}`}>{STATUS_LABEL[po.status] ?? po.status}</span>
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-md ${STATUS_CHIP[po.status] ?? 'bg-surface-container text-on-surface-variant'}`}>{STATUS_LABEL[po.status] ?? po.status}</span>
                      ...
                      {po.stakeholders?.category && <p className="text-[11px] text-on-surface-variant/40 mt-0.5">{po.stakeholders.category}</p>}
                      {getItemsPreview(po) && <p className="text-[11px] text-on-surface-variant/30 mt-1 line-clamp-1">{getItemsPreview(po)}</p>}
                    </div>
                    <div className="text-right shrink-0 pt-0.5">
                      {Number(po.vendor_bill_amount) > 0 ? (
                        <><p className="font-mono text-[14px] font-bold text-green-700 tabular-nums">₹{Number(po.vendor_bill_amount).toLocaleString('en-IN')}</p><p className="text-[10px] text-on-surface-variant/35 mt-0.5">billed</p></>
                      ) : (Number(po.total_value) > 0 || Number(po.order_value) > 0) ? (
                        <><p className="font-mono text-[14px] font-semibold text-on-surface/60 tabular-nums">₹{(Number(po.total_value) || Number(po.order_value)).toLocaleString('en-IN')}</p><p className="text-[10px] text-amber-500 mt-0.5">no bill</p></>
                      ) : null}
                    </div>
                  </div>
                </div>
                <div className="flex items-center justify-between px-4 py-2 border-t border-outline-variant/[0.05]">
                  <p className="text-[11px] text-on-surface-variant/45 truncate flex-1">{fmtDate(po.date_issued)}</p>
                  {po.expected_delivery && <p className="text-[11px] text-on-surface-variant/30 ml-3 shrink-0"><span className={isOverdue(po) ? 'text-red-400' : ''}>{fmtShortDate(po.expected_delivery)}</span></p>}
                </div>
              </div>
            ))}
          </div>
