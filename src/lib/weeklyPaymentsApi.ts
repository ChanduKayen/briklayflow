...
export async function loadWeeklyPayments(monday: Date): Promise<WeeklyPayments> {
  ...
  if (isCurrentWeek) try {
    // Per-SITE carry-forward — the ONE source is v_party_site_balance: what each worker is owed ON each
    // project (opening by-site + certified + wages there − payments there = that site's "ahead"). The rule,
    // per site: balance b/f = that site's owed − this week's payable on it. It folds into the worker's row
    // on that site, or (no work there this week) a standalone row tagged to that site. NO party-level
    // top-up: the site figures are authoritative, so an advance on one site never inflates another.
    const siteR = await supabase.from('v_party_site_balance').select('stakeholder_id, project_id, to_pay').gt('to_pay', 0);
    ...
    const owedIds = Object.keys(siteOwed);
    if (owedIds.length) {
      // This week's payable already booked per (worker, site) = b/f + this week; and the row a site's
      // earlier balance folds into.
      ...
      sections.forEach(s => s.rows.forEach(r => {
        ...
        (repBySite[r.stakeholderId] ||= {})[r.projectId] = ((repBySite[r.stakeholderId] ||= {})[r.projectId] || 0) + (r.balanceBf + r.thisWeek);
