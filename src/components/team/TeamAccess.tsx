...
interface Member { id: string; name: string; role: UserRole; assigned_projects: string[]; can_approve_procurement: boolean; procurement_approval_limit: number | null; higher_approver_id: string | null; can_certify_work: boolean; work_certification_limit: number | null }
...
function MemberCard({
  ...
  onProcApproval: (updates: { can_approve_procurement?: boolean; procurement_approval_limit?: number | null; higher_approver_id?: string | null; can_certify_work?: boolean; work_certification_limit?: number | null }) => void;
}) {
  const { show } = useSnackbar();
  ...
  return (
    <div className={`rounded-2xl p-4 db-card ${phase === 'saving' ? 'db-glow' : ''}`}
      ...
      {/* Approvals grid — the governance primitives. Procurement + Work certification (one row each;
          Payments is the natural next column). Each: a toggle, an optional ₹ cap, and an escalation. */}
      <p className="mt-3 mb-1" style={{ color: V.faint, ...font, ...T.xs, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Approvals</p>
      <div className="flex flex-col gap-2">
        <ApprovalRow
          label="Can approve purchase requests" on={member.can_approve_procurement}
          limit={member.procurement_approval_limit} higherId={member.higher_approver_id} approvers={approvers}
          onToggle={() => onProcApproval({ can_approve_procurement: !member.can_approve_procurement })}
          onLimit={(v) => onProcApproval({ procurement_approval_limit: v })}
          onHigher={(id) => onProcApproval({ higher_approver_id: id })}
        />
        <ApprovalRow
          label="Can certify site work" on={member.can_certify_work}
          limit={member.work_certification_limit} higherId={member.higher_approver_id} approvers={approvers}
          higherNote="the project's Works Approver signs off first"
          onToggle={() => onProcApproval({ can_certify_work: !member.can_certify_work })}
          onLimit={(v) => onProcApproval({ work_certification_limit: v })}
          onHigher={(id) => onProcApproval({ higher_approver_id: id })}
        />
      </div>
    </div>
...
// One governance row in the Approvals grid — a toggle + an optional ₹ cap + an escalation target.
function ApprovalRow({ label, on, limit, higherId, approvers, higherNote, onToggle, onLimit, onHigher }: {
  label: string; on: boolean; limit: number | null; higherId: string | null;
  approvers: { id: string; name: string }[]; higherNote?: string;
  onToggle: () => void; onLimit: (v: number | null) => void; onHigher: (id: string | null) => void;
}) {
  return (
    <div className="flex items-center gap-2.5 rounded-xl px-3 py-2.5" style={{ background: on ? V.terraWash : V.field }}>
      <div className="flex-1 min-w-0">
        <p style={{ color: V.inkSoft, ...font, ...T.xs }}>{label}</p>
        {on && (
          <div className="mt-1.5 flex flex-col gap-1.5">
            <div className="inline-flex items-center gap-1.5">
              <span style={{ color: V.faint, ...font, ...T.xs }}>up to ₹</span>
              <input inputMode="numeric" defaultValue={limit ?? ''} placeholder="no limit"
                onBlur={(e) => { const v = e.target.value.replace(/[^\d.]/g, ''); onLimit(v ? parseFloat(v) : null); }}
            ...
            <div className="inline-flex items-center gap-1.5 flex-wrap">
              <span style={{ color: V.faint, ...font, ...T.xs }}>{higherNote ? 'over that, ' + higherNote + '; else ask' : 'over that, ask'}</span>
              <select value={higherId ?? ''} onChange={(e) => onHigher(e.target.value || null)}
                className="px-2 py-0.5 rounded outline-none"
                style={{ background: V.surface, border: `1px solid ${V.line}`, color: higherId ? V.ink : V.faint, ...font, ...T.xs, maxWidth: 180 }}>
      ...
      <button onClick={onToggle} aria-label={`Toggle ${label}`} className="w-9 h-5 rounded-full relative shrink-0"
        style={{ background: on ? V.terra : V.line, transition: 'background .18s ease' }}>
        <span className="absolute top-0.5 w-4 h-4 rounded-full" style={{ background: '#fff', left: on ? 18 : 2, transition: 'left .18s cubic-bezier(.3,.8,.3,1.2)', boxShadow: '0 1px 2px rgba(0,0,0,0.18)' }} />
      </button>
    </div>
  );
}
...
export default function TeamAccess({ session }: { session: Session }) {
  ...
  const { data: members = [], isLoading: membersLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: rows, error } = await supabase
        .from('org_memberships').select('role, status, user_id, joined_at, can_approve_procurement, procurement_approval_limit, higher_approver_id, can_certify_work, work_certification_limit')
      ...
      return (rows ?? []).map((r: any) => {
        ...
        return { id: r.user_id, name: p?.name ?? 'Unnamed', role: (p?.role ?? r.role) as UserRole, assigned_projects: (p?.assigned_projects ?? []) as string[], can_approve_procurement: !!r.can_approve_procurement, procurement_approval_limit: r.procurement_approval_limit ?? null, higher_approver_id: r.higher_approver_id ?? null, can_certify_work: !!r.can_certify_work, work_certification_limit: r.work_certification_limit ?? null } satisfies Member;
      });
    },
  });
  ...
  const setProcApproval = (m: Member, updates: { can_approve_procurement?: boolean; procurement_approval_limit?: number | null; higher_approver_id?: string | null; can_certify_work?: boolean; work_certification_limit?: number | null }) => {
            ...
            <div className="space-y-2.5">
              {members.map((m) => (
                <MemberCard key={m.id} member={m} sender={senderForMember(m)} suggestPhone={suggestPhoneFor(m)} projects={projects}
                  ...
                  onProcApproval={(updates) => setProcApproval(m, updates)}
                />
              ))}
            </div>
          )}
