  ...
  useEffect(() => {
    ...
    function itemHTML(it: Row, e: string) {
      ...
      return '<div class="ihead"><span class="no">' + (i + 1) + '</span><div class="t">' +
        ...
        line('size', 'Size', sizeV, 'Tap to add size') + field('size', 'Size', '<div class="two"><input class="inp" data-f="w" type="text" inputmode="numeric" placeholder="Width" value="' + esc(it.w) + '" aria-label="Width in mm"><span>×</span><input class="inp" data-f="h" type="text" inputmode="numeric" placeholder="Height" value="' + esc(it.h) + '" aria-label="Height in mm"><span>mm</span></div>') +
        line('spec', 'Specification', esc(it.spec), 'Tap to add specification') + field('spec', 'Specification', inp('spec', '')) +
        (it.brand || e === 'brand' ? line('brand', 'Brand', esc(it.brand), '') + (e === 'brand' ? '<div class="edwrap">' + field('brand', 'Brand', inp('brand', '')) + '<button type="button" class="sw" data-brandall aria-pressed="' + (P && P.brandAll) + '"><i></i>Same for all ' + n + ' items</button></div>' : '') : '') +
        (it.note || e === 'note' ? line('note', 'Note', esc(it.note), '') + field('note', 'Note', inp('note', 'Note for the vendor')) : '') +
